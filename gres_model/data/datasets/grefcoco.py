...
def load_grefcoco_json(refer_root, dataset_name, splitby, split, image_root, extra_annotation_keys=None, extra_refer_keys=None):
    ...
    cat_mapid = json.load(open(os.path.join(refer_root, dataset_name, "cateid2coco.json")))
    cat_supmap = json.load(open(os.path.join(refer_root, dataset_name, "cocoidtosuper.json")))
    ...
    for (img_dict, ref_dict, anno_dicts) in imgs_refs_anns:
        ...
        if None in anno_dicts:
            ...
            obj["empty"] = True
            obj["category_id"] = ref_dict["category_id"]
            obj = [obj]
        ...
        category_id = ref_dict["category_id"]
        # category_id = [cat_mapid[str(cate)] for cate in category_id]
        category_id = [cat_supmap[str(cat_mapid[str(cate)])][0] for cate in category_id]
        record["category_id"] = category_id #get the category_id [-1] nothing 
...
if __name__ == "__main__":
    ...
    REFCOCO_PATH = '//lustre/hhding/code/ReLA/datasets'
