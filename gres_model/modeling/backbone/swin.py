...
class MultiModalSwinTransformer(nn.Module):
    ...
    def forward(self, x, l, l_mask):
        ...
        # outs["lang"]= l #language fused after
        return outs
...
class MMBasicLayer(nn.Module):
    ...
    def forward(self, x, H, W, l, l_mask):
        ...
        if self.downsample is not None:
            ...
            return x_residual, H, W, None, H, W
...
class PWAM(nn.Module):
    def __init__(self, dim, v_in_channels, l_in_channels, key_channels, value_channels, num_heads=0, dropout=0.0):
        ...
        self.image_lang_att = SpatialImageLanguageAttention(v_in_channels,  # v_in
                                                            ...
                                                            num_heads=num_heads)
        # self.lang_image_att = LanguageAttention(v_in_channels, l_in_channels, l_in_channels, l_in_channels, out_channels=l_in_channels, num_heads=num_heads)
        ...
        #     nn.Linear(l_in_channels, l_in_channels),
        #     nn.Sigmoid()
        # )

        # self.project_lm = nn.Sequential(
        #     nn.Conv1d(l_in_channels, l_in_channels, 1, 1),
        #     nn.GELU(),
        #     nn.Dropout(dropout)
        # )
    def forward(self, x, l, l_mask):
        # input x shape: (B, H*W, dim)
        # input l shape: (B, dim, n_l)
        #----------------------------------------------------------------------- visual path
        vis = self.vis_project(x.permute(0, 2, 1))  # (B, dim, H*W)

        vis_lang = self.image_lang_att(x, l, l_mask)  # (B, H*W, dim)

        vis_lang = vis_lang.permute(0, 2, 1)  # (B, dim, H*W)

        mm = torch.mul(vis, vis_lang)
        mm = self.project_mm(mm)  # (B, dim, H*W)

        mm = mm.permute(0, 2, 1)  # (B, H*W, dim)
        #----------------------------------------------------------------------- Language path
        # lang = self.project_lm(l)

        # lang_vis = self.lang_image_att(x, l, l_mask) #(B dim n_l)

        # lang_fuse = rearrange((lang + lang_vis), "b d l -> b l d")
        
        # lm_gated = rearrange(self.gated_proj(lang_fuse), "b l d -> b d l")

        # lm = lm_gated * lang + (1 - lm_gated) * lang_vis #增加gated
        
        # lm = self.project_lm(lm)
        return mm
...
class LanguageAttention(nn.Module):
    def __init__(self, v_in_channels, l_in_channels, key_channels, value_channels, out_channels=None, num_heads=1):
        ...
        if out_channels is None:
            self.out_channels = value_channels
    ...
    def forward(self, x, l, l_mask):
        ...
        n_l = l.shape[2]
        query = self.f_query(l)  # (B, key_channels, N_l) if Conv1D
        query = query.permute(0, 2, 1)  # (B, N_l, key_channels)
        key = self.f_key(x)  # (B, key_channels, HW)
        value = self.f_value(x)  # (B, self.value_channels, HW)
        ...
        sim_map = torch.matmul(query, key)  # (B, self.num_heads, N_l, HW) 
...
class D2SwinTransformer(MultiModalSwinTransformer, Backbone):
